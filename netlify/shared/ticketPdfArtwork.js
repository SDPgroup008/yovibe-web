...
function renderSvg(data, options = {}) {
  ...
  const bg = design.source === 'upload' && design.background_url ? `<image href="${href(design.background_url)}" x="${layout.bgTransform.x || 0}" y="${layout.bgTransform.y || 0}" width="${W * (layout.bgTransform.scale || 1)}" height="${H * (layout.bgTransform.scale || 1)}" preserveAspectRatio="xMidYMid slice" opacity=".85"/>` : `<defs><linearGradient id="bg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="${base}"/><stop offset="100%" stop-color="${accent}" stop-opacity=".72"/></linearGradient><clipPath id="posterClip"><rect width="${poster.width}" height="${poster.height}" rx="10"/></clipPath></defs><rect width="${W}" height="${H}" fill="url(#bg)"/>`;
