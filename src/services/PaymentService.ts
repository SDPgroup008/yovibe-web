...
class PaymentService {
  ...
  private async getToken(apiType: "collection" | "disbursement"): Promise<string> {
    const tokenKey = `${apiType}Token` as keyof this
    const token = this[tokenKey] as string | null

    if (!token || this.isTokenExpired(token)) {
      const newToken = await this.generateAccessToken(apiType)
      ;(this[tokenKey] as string | null) = newToken
      console.log(`${apiType} token refreshed`)
    }

    return this[tokenKey] as string
  }
  ...
  validatePaymentAccount(method: PaymentMethod, accountNumber: string): { valid: boolean; message: string } {
    switch (method) {
      case "mtn":
        return this.validateMTNPhoneNumber(accountNumber)
          ? { valid: true, message: "Valid MTN number" }
          : { valid: false, message: "Invalid MTN number format" }
      case "airtel":
        return this.validateAirtelPhoneNumber(accountNumber)
          ? { valid: true, message: "Valid Airtel number" }
          : { valid: false, message: "Invalid Airtel number format" }
      case "card":
        return /^\d{16}$/.test(accountNumber.replace(/\s/g, ""))
          ? { valid: true, message: "Valid card number" }
          : { valid: false, message: "Invalid card number format" }
      default:
